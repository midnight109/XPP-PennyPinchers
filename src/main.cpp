...
const string strMessageMagic = "Pennypincher Signed Message:\n";
...
{
    // Pennypincher: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 648000;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Pennypincher: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Pennypincher: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Pennypincher: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PennypincherMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PennypincherMiner : ProcessBlock, block not accepted");
    }
...
void static LitecoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running Pennypincher with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PennypincherMiner terminated\n");
        throw;
    }
}
